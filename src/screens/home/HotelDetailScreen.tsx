...
export default function HotelDetailScreen({ navigation, route }: Props) {
  ...
  const loadDetail = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/api/v1/lodgings/${lodgingId}`, {
        headers: requestHeaders,
        signal,
      });
      if (!response.ok) {
        throw new Error(await getErrorMessage(response, `숙소 상세 요청 실패 (${response.status})`));
      }
      const data: LodgingDetailResponse = await response.json();
      if (!signal?.aborted) setDetail(data);
    } catch (loadError) {
      if (!signal?.aborted) {
        setError(loadError instanceof Error ? loadError.message : '숙소 상세 정보를 불러오지 못했습니다.');
      }
    } finally {
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      <View style={[styles.header, desktop && styles.desktopHeader]}>
        ...
        <View style={styles.headerTextBox}>
          ...
          <Text style={styles.headerSub}>숙소 상세 정보</Text>
        </View>
      </View>
      ...
      >
        {imageUrls.length > 0 ? (
          ...
          <View style={[styles.imagePlaceholder, { width: imageWidth, height: heroImageHeight }]}>
            <Ionicons name="bed-outline" size={52} color={COLORS.textMuted} />
        ...
        <View style={styles.titleBlock}>
          ...
          <View style={styles.inlineInfo}>
            ...
            <Text style={styles.inlineInfoText}>{address ?? '주소 정보 없음'}</Text>
          </View>
        ...
        <View style={styles.section}>
          ...
          <InfoRow label="객실 수" value={detail?.roomCount} />
          <InfoRow label="객실 유형" value={detail?.roomType} />
          <InfoRow label="체크인" value={detail?.checkInTime} />
          <InfoRow label="체크아웃" value={detail?.checkOutTime} />
          <InfoRow label="주차" value={detail?.parking} />
          <InfoRow label="부대시설" value={detail?.subFacility} />
          <InfoRow label="문의" value={detail?.infoCenter} />
        </View>

        <View style={styles.section}>
          <Text style={styles.sectionTitle}>소개</Text>
          <Text style={[styles.description, !detail?.description && styles.infoValueMuted]}>
            {detail?.description || INFO_FALLBACK}
          </Text>
        </View>
